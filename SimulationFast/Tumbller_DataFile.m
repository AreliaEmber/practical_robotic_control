% Simscape(TM) Multibody(TM) version: 24.1

% This is a model data file derived from a Simscape Multibody Import XML file using the smimport function.
% The data in this file sets the block parameter values in an imported Simscape Multibody model.
% For more information on this file, see the smimport function help page in the Simscape Multibody documentation.
% You can modify numerical values, but avoid any other changes to this file.
% Do not add code to this file. Do not edit the physical units shown in comments.

%%%VariableName:smiData


%============= RigidTransform =============%

%Initialize the RigidTransform structure array by filling in null values.
smiData.RigidTransform(58).translation = [0.0 0.0 0.0];
smiData.RigidTransform(58).angle = 0.0;
smiData.RigidTransform(58).axis = [0.0 0.0 0.0];
smiData.RigidTransform(58).ID = "";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(1).translation = [6.2019019999999996 -175.39327299999999 3];  % mm
smiData.RigidTransform(1).angle = 3.1415926535897931;  % rad
smiData.RigidTransform(1).axis = [1 0 0];
smiData.RigidTransform(1).ID = "B[M/B1sYP9MzMsANU8+:-:]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(2).translation = [0 0 0];  % mm
smiData.RigidTransform(2).angle = 0;  % rad
smiData.RigidTransform(2).axis = [0 0 0];
smiData.RigidTransform(2).ID = "F[M/B1sYP9MzMsANU8+:-:]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(3).translation = [1.1190410000000002 -0.82562000000000002 -5.3226550000000001];  % mm
smiData.RigidTransform(3).angle = 3.1415926535897931;  % rad
smiData.RigidTransform(3).axis = [0 0 1];
smiData.RigidTransform(3).ID = "B[:-:MAb1FE2LcQFTT1qsM]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(4).translation = [12.193894 53.907218 7];  % mm
smiData.RigidTransform(4).angle = 0;  % rad
smiData.RigidTransform(4).axis = [0 0 0];
smiData.RigidTransform(4).ID = "F[:-:MAb1FE2LcQFTT1qsM]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(5).translation = [-14.703097999999999 -138.290222 3];  % mm
smiData.RigidTransform(5).angle = 3.1415926535897931;  % rad
smiData.RigidTransform(5).axis = [1 0 0];
smiData.RigidTransform(5).ID = "B[M/B1sYP9MzMsANU8+:-:M5EnXQOWFqi788iB8]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(6).translation = [0 0 -9];  % mm
smiData.RigidTransform(6).angle = 3.1415926535897931;  % rad
smiData.RigidTransform(6).axis = [1 0 0];
smiData.RigidTransform(6).ID = "F[M/B1sYP9MzMsANU8+:-:M5EnXQOWFqi788iB8]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(7).translation = [6.2019019999999996 -175.39327299999999 -10];  % mm
smiData.RigidTransform(7).angle = 3.1415926535897931;  % rad
smiData.RigidTransform(7).axis = [1 0 0];
smiData.RigidTransform(7).ID = "B[M/B1sYP9MzMsANU8+:-:MAb1FE2LcQFTT1qsM]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(8).translation = [12.193894 24.722151 2];  % mm
smiData.RigidTransform(8).angle = 0;  % rad
smiData.RigidTransform(8).axis = [0 0 0];
smiData.RigidTransform(8).ID = "F[M/B1sYP9MzMsANU8+:-:MAb1FE2LcQFTT1qsM]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(9).translation = [0 -0 45];  % mm
smiData.RigidTransform(9).angle = 3.1415926535897931;  % rad
smiData.RigidTransform(9).axis = [1 0 0];
smiData.RigidTransform(9).ID = "B[MB0wE8w91BCZA9uYt:-:M/B1sYP9MzMsANU8+]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(10).translation = [61.696901999999994 -139.390209 0];  % mm
smiData.RigidTransform(10).angle = 3.1415926535897931;  % rad
smiData.RigidTransform(10).axis = [1 0 0];
smiData.RigidTransform(10).ID = "F[MB0wE8w91BCZA9uYt:-:M/B1sYP9MzMsANU8+]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(11).translation = [67.196898000000004 -175.39021399999999 3];  % mm
smiData.RigidTransform(11).angle = 2.0943951023931953;  % rad
smiData.RigidTransform(11).axis = [0.57735026918962584 0.57735026918962584 0.57735026918962584];
smiData.RigidTransform(11).ID = "B[M/B1sYP9MzMsANU8+:-:MCaySsSuHHCcX/L7H]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(12).translation = [0 0.001108 0];  % mm
smiData.RigidTransform(12).angle = 2.0943951023931953;  % rad
smiData.RigidTransform(12).axis = [-0.57735026918962584 -0.57735026918962584 0.57735026918962584];
smiData.RigidTransform(12).ID = "F[M/B1sYP9MzMsANU8+:-:MCaySsSuHHCcX/L7H]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(13).translation = [0 0 0];  % mm
smiData.RigidTransform(13).angle = 3.1415926535897931;  % rad
smiData.RigidTransform(13).axis = [1 0 0];
smiData.RigidTransform(13).ID = "B[MCjzoZVZszPmYWC+N:-:M/B1sYP9MzMsANU8+]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(14).translation = [38.702041999999999 -207.32060099999998 0];  % mm
smiData.RigidTransform(14).angle = 3.1415926535897931;  % rad
smiData.RigidTransform(14).axis = [1 0 0];
smiData.RigidTransform(14).ID = "F[MCjzoZVZszPmYWC+N:-:M/B1sYP9MzMsANU8+]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(15).translation = [0 0 -9];  % mm
smiData.RigidTransform(15).angle = 0;  % rad
smiData.RigidTransform(15).axis = [0 0 0];
smiData.RigidTransform(15).ID = "B[MM9uKgPjrwJIzRyRw:-:M/B1sYP9MzMsANU8+]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(16).translation = [-26.308235999999997 -207.32060099999998 0];  % mm
smiData.RigidTransform(16).angle = 3.1415926535897931;  % rad
smiData.RigidTransform(16).axis = [1 0 0];
smiData.RigidTransform(16).ID = "F[MM9uKgPjrwJIzRyRw:-:M/B1sYP9MzMsANU8+]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(17).translation = [0 -0 45];  % mm
smiData.RigidTransform(17).angle = 3.1415926535897931;  % rad
smiData.RigidTransform(17).axis = [1 0 0];
smiData.RigidTransform(17).ID = "B[MSzXYclwIl/wNsWfw:-:M/B1sYP9MzMsANU8+]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(18).translation = [61.696906000000006 -211.39021099999999 0];  % mm
smiData.RigidTransform(18).angle = 3.1415926535897931;  % rad
smiData.RigidTransform(18).axis = [1 0 0];
smiData.RigidTransform(18).ID = "F[MSzXYclwIl/wNsWfw:-:M/B1sYP9MzMsANU8+]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(19).translation = [0 0 -9];  % mm
smiData.RigidTransform(19).angle = 0;  % rad
smiData.RigidTransform(19).axis = [0 0 0];
smiData.RigidTransform(19).ID = "B[MavxK54vt3HBa5iBX:-:M/B1sYP9MzMsANU8+]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(20).translation = [38.702047 -143.459836 0];  % mm
smiData.RigidTransform(20).angle = 3.1415926535897931;  % rad
smiData.RigidTransform(20).axis = [1 0 0];
smiData.RigidTransform(20).ID = "F[MavxK54vt3HBa5iBX:-:M/B1sYP9MzMsANU8+]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(21).translation = [-26.308246 -143.45983100000001 0];  % mm
smiData.RigidTransform(21).angle = 0;  % rad
smiData.RigidTransform(21).axis = [0 0 0];
smiData.RigidTransform(21).ID = "B[M/B1sYP9MzMsANU8+:-:Mb70ykak2Au8T6fCp]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(22).translation = [0 0 0];  % mm
smiData.RigidTransform(22).angle = 0;  % rad
smiData.RigidTransform(22).axis = [0 0 0];
smiData.RigidTransform(22).ID = "F[M/B1sYP9MzMsANU8+:-:Mb70ykak2Au8T6fCp]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(23).translation = [0 -0 45];  % mm
smiData.RigidTransform(23).angle = 3.1415926535897931;  % rad
smiData.RigidTransform(23).axis = [1 0 0];
smiData.RigidTransform(23).ID = "B[MeN4xHxtNaF8BNQeM:-:M/B1sYP9MzMsANU8+]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(24).translation = [-49.303103 -211.390209 0];  % mm
smiData.RigidTransform(24).angle = 3.1415926535897931;  % rad
smiData.RigidTransform(24).axis = [1 0 0];
smiData.RigidTransform(24).ID = "F[MeN4xHxtNaF8BNQeM:-:M/B1sYP9MzMsANU8+]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(25).translation = [0 0 0];  % mm
smiData.RigidTransform(25).angle = 3.1415926535897931;  % rad
smiData.RigidTransform(25).axis = [1 0 0];
smiData.RigidTransform(25).ID = "B[Mq6CqRBUVohgnO0dn:-:M/B1sYP9MzMsANU8+]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(26).translation = [27.096905 -138.290209 3];  % mm
smiData.RigidTransform(26).angle = 0;  % rad
smiData.RigidTransform(26).axis = [0 0 0];
smiData.RigidTransform(26).ID = "F[Mq6CqRBUVohgnO0dn:-:M/B1sYP9MzMsANU8+]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(27).translation = [0 0.001108 1.5];  % mm
smiData.RigidTransform(27).angle = 2.0943951023931953;  % rad
smiData.RigidTransform(27).axis = [0.57735026918962584 -0.57735026918962584 -0.57735026918962584];
smiData.RigidTransform(27).ID = "B[MwFa5EJ28porNxG9e:-:M/B1sYP9MzMsANU8+]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(28).translation = [-54.803096000000004 -175.39021399999999 1.5];  % mm
smiData.RigidTransform(28).angle = 2.0943951023931953;  % rad
smiData.RigidTransform(28).axis = [-0.57735026918962584 -0.57735026918962584 0.57735026918962584];
smiData.RigidTransform(28).ID = "F[MwFa5EJ28porNxG9e:-:M/B1sYP9MzMsANU8+]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(29).translation = [-49.303094000000002 -139.39021500000001 0];  % mm
smiData.RigidTransform(29).angle = 0;  % rad
smiData.RigidTransform(29).axis = [0 0 0];
smiData.RigidTransform(29).ID = "B[M/B1sYP9MzMsANU8+:-:MyTlWXuhsDfq6OZmW]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(30).translation = [0 -0 45];  % mm
smiData.RigidTransform(30).angle = 0;  % rad
smiData.RigidTransform(30).axis = [0 0 0];
smiData.RigidTransform(30).ID = "F[M/B1sYP9MzMsANU8+:-:MyTlWXuhsDfq6OZmW]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(31).translation = [-5 -0 -33];  % mm
smiData.RigidTransform(31).angle = 2.0943951023931953;  % rad
smiData.RigidTransform(31).axis = [0.57735026918962584 0.57735026918962584 0.57735026918962584];
smiData.RigidTransform(31).ID = "B[M/JqJcB5Lk7hNWPPR:-:Mi7OlT21hijGcVQe1:M/RGs/Ph4e0yE1FFn]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(32).translation = [-8.0219889999999996 -53 -5.080152];  % mm
smiData.RigidTransform(32).angle = 2.0943951023931953;  % rad
smiData.RigidTransform(32).axis = [-0.57735026918962584 -0.57735026918962584 0.57735026918962584];
smiData.RigidTransform(32).ID = "F[M/JqJcB5Lk7hNWPPR:-:Mi7OlT21hijGcVQe1:M/RGs/Ph4e0yE1FFn]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(33).translation = [3 -0 -33];  % mm
smiData.RigidTransform(33).angle = 2.0943951023931953;  % rad
smiData.RigidTransform(33).axis = [0.57735026918962584 0.57735026918962584 0.57735026918962584];
smiData.RigidTransform(33).ID = "B[M/JqJcB5Lk7hNWPPR:-:MwFa5EJ28porNxG9e]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(34).translation = [3 -0 -33];  % mm
smiData.RigidTransform(34).angle = 2.0943951023931953;  % rad
smiData.RigidTransform(34).axis = [0.57735026918962584 0.57735026918962584 0.57735026918962584];
smiData.RigidTransform(34).ID = "F[M/JqJcB5Lk7hNWPPR:-:MwFa5EJ28porNxG9e]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(35).translation = [0 0 0];  % mm
smiData.RigidTransform(35).angle = 3.1415926535897931;  % rad
smiData.RigidTransform(35).axis = [1 0 0];
smiData.RigidTransform(35).ID = "B[M5EnXQOWFqi788iB8:-:MGYEl8QP/DgkWqKIc]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(36).translation = [-14.703097 -3 -212.490218];  % mm
smiData.RigidTransform(36).angle = 1.5707963267948968;  % rad
smiData.RigidTransform(36).axis = [1 0 0];
smiData.RigidTransform(36).ID = "F[M5EnXQOWFqi788iB8:-:MGYEl8QP/DgkWqKIc]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(37).translation = [3 -0 -33];  % mm
smiData.RigidTransform(37).angle = 2.0943951023931953;  % rad
smiData.RigidTransform(37).axis = [0.57735026918962584 0.57735026918962584 0.57735026918962584];
smiData.RigidTransform(37).ID = "B[MI2dF8WfaHT4dwwx7:-:MCaySsSuHHCcX/L7H]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(38).translation = [3 -0 -33];  % mm
smiData.RigidTransform(38).angle = 2.0943951023931953;  % rad
smiData.RigidTransform(38).axis = [0.57735026918962584 0.57735026918962584 0.57735026918962584];
smiData.RigidTransform(38).ID = "F[MI2dF8WfaHT4dwwx7:-:MCaySsSuHHCcX/L7H]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(39).translation = [-12.5 0 0];  % mm
smiData.RigidTransform(39).angle = 1.5707963267948968;  % rad
smiData.RigidTransform(39).axis = [0 1 0];
smiData.RigidTransform(39).ID = "B[MFPMcNY1/xfSkVter:-:MqwbMUvw+UAzAxcIl]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(40).translation = [-49.303103 -211.390209 3];  % mm
smiData.RigidTransform(40).angle = 0;  % rad
smiData.RigidTransform(40).axis = [0 0 0];
smiData.RigidTransform(40).ID = "F[MFPMcNY1/xfSkVter:-:MqwbMUvw+UAzAxcIl]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(41).translation = [6.2021860000000002 -177.45083600000001 3];  % mm
smiData.RigidTransform(41).angle = 3.1415926535897931;  % rad
smiData.RigidTransform(41).axis = [0 1 0];
smiData.RigidTransform(41).ID = "B[MqwbMUvw+UAzAxcIl:-:MFqZmU3AllKkc8ndY]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(42).translation = [40 -25 20];  % mm
smiData.RigidTransform(42).angle = 0;  % rad
smiData.RigidTransform(42).axis = [0 0 0];
smiData.RigidTransform(42).ID = "F[MqwbMUvw+UAzAxcIl:-:MFqZmU3AllKkc8ndY]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(43).translation = [-5 -0 -33];  % mm
smiData.RigidTransform(43).angle = 2.0943951023931953;  % rad
smiData.RigidTransform(43).axis = [0.57735026918962584 0.57735026918962584 0.57735026918962584];
smiData.RigidTransform(43).ID = "B[MI2dF8WfaHT4dwwx7:-:Mmd4uProBNDr4dXwC]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(44).translation = [-8.0219889999999996 -53 -5.080152];  % mm
smiData.RigidTransform(44).angle = 2.0943951023931953;  % rad
smiData.RigidTransform(44).axis = [-0.57735026918962584 -0.57735026918962584 0.57735026918962584];
smiData.RigidTransform(44).ID = "F[MI2dF8WfaHT4dwwx7:-:Mmd4uProBNDr4dXwC]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(45).translation = [-49.303094000000002 -139.39021500000001 3];  % mm
smiData.RigidTransform(45).angle = 3.1415926535897931;  % rad
smiData.RigidTransform(45).axis = [-0.70710678118654746 0.70710678118654757 0];
smiData.RigidTransform(45).ID = "B[MqwbMUvw+UAzAxcIl:-:MILpt9V65mBV5VXDx]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(46).translation = [12.5 0 0];  % mm
smiData.RigidTransform(46).angle = 2.0943951023931953;  % rad
smiData.RigidTransform(46).axis = [0.57735026918962584 0.57735026918962584 0.57735026918962584];
smiData.RigidTransform(46).ID = "F[MqwbMUvw+UAzAxcIl:-:MILpt9V65mBV5VXDx]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(47).translation = [-12.5 0 0];  % mm
smiData.RigidTransform(47).angle = 1.5707963267948968;  % rad
smiData.RigidTransform(47).axis = [-0 -1 -0];
smiData.RigidTransform(47).ID = "B[MN/UArdgi/EGndnbW:-:Mq3EWHAOdjBiY1BQa]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(48).translation = [-49.303094000000002 -139.39021500000001 3];  % mm
smiData.RigidTransform(48).angle = 0;  % rad
smiData.RigidTransform(48).axis = [0 0 0];
smiData.RigidTransform(48).ID = "F[MN/UArdgi/EGndnbW:-:Mq3EWHAOdjBiY1BQa]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(49).translation = [12.5 0 0];  % mm
smiData.RigidTransform(49).angle = 3.1415926535897931;  % rad
smiData.RigidTransform(49).axis = [-0.70710678118654746 0 0.70710678118654757];
smiData.RigidTransform(49).ID = "B[MN/UArdgi/EGndnbW:-:MqwbMUvw+UAzAxcIl]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(50).translation = [61.696906000000006 -211.39021099999999 3];  % mm
smiData.RigidTransform(50).angle = 0;  % rad
smiData.RigidTransform(50).axis = [0 0 0];
smiData.RigidTransform(50).ID = "F[MN/UArdgi/EGndnbW:-:MqwbMUvw+UAzAxcIl]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(51).translation = [21.678011000000001 -53 -5.080152];  % mm
smiData.RigidTransform(51).angle = 2.0943951023931953;  % rad
smiData.RigidTransform(51).axis = [0.57735026918962584 0.57735026918962584 0.57735026918962584];
smiData.RigidTransform(51).ID = "B[Mi7OlT21hijGcVQe1:M/RGs/Ph4e0yE1FFn:-:Mi7OlT21hijGcVQe1:M5XlKUBcJRDhDNUXr]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(52).translation = [21.978010999999999 -53 -5.080152];  % mm
smiData.RigidTransform(52).angle = 2.0943951023931953;  % rad
smiData.RigidTransform(52).axis = [0.57735026918962584 0.57735026918962584 0.57735026918962584];
smiData.RigidTransform(52).ID = "F[Mi7OlT21hijGcVQe1:M/RGs/Ph4e0yE1FFn:-:Mi7OlT21hijGcVQe1:M5XlKUBcJRDhDNUXr]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(53).translation = [21.678011000000001 -53 -5.080152];  % mm
smiData.RigidTransform(53).angle = 2.0943951023931953;  % rad
smiData.RigidTransform(53).axis = [0.57735026918962584 0.57735026918962584 0.57735026918962584];
smiData.RigidTransform(53).ID = "B[Mmd4uProBNDr4dXwC:-:MwAEj9l1/8GsKP32G]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(54).translation = [21.978010999999999 -53 -5.080152];  % mm
smiData.RigidTransform(54).angle = 2.0943951023931953;  % rad
smiData.RigidTransform(54).axis = [0.57735026918962584 0.57735026918962584 0.57735026918962584];
smiData.RigidTransform(54).ID = "F[Mmd4uProBNDr4dXwC:-:MwAEj9l1/8GsKP32G]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(55).translation = [61.696901999999994 -139.390209 3];  % mm
smiData.RigidTransform(55).angle = 3.1415926535897931;  % rad
smiData.RigidTransform(55).axis = [0.70710678118654746 0.70710678118654757 0];
smiData.RigidTransform(55).ID = "B[MqwbMUvw+UAzAxcIl:-:MpxKCApZ+X0anvG8J]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(56).translation = [-12.5 0 -0];  % mm
smiData.RigidTransform(56).angle = 2.0943951023931953;  % rad
smiData.RigidTransform(56).axis = [-0.57735026918962584 -0.57735026918962584 0.57735026918962584];
smiData.RigidTransform(56).ID = "F[MqwbMUvw+UAzAxcIl:-:MpxKCApZ+X0anvG8J]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(57).translation = [0 -0 0];  % mm
smiData.RigidTransform(57).angle = 3.1415926535897931;  % rad
smiData.RigidTransform(57).axis = [-0 -0 1];
smiData.RigidTransform(57).ID = "B[MyTlWXuhsDfq6OZmW:-:MqwbMUvw+UAzAxcIl]";

%Translation Method - Cartesian
%Rotation Method - Arbitrary Axis
smiData.RigidTransform(58).translation = [61.696901999999994 -139.390209 0];  % mm
smiData.RigidTransform(58).angle = 3.1415926535897931;  % rad
smiData.RigidTransform(58).axis = [1 0 0];
smiData.RigidTransform(58).ID = "F[MyTlWXuhsDfq6OZmW:-:MqwbMUvw+UAzAxcIl]";


%============= Solid =============%
%Center of Mass (CoM) %Moments of Inertia (MoI) %Product of Inertia (PoI)

%Initialize the Solid structure array by filling in null values.
%Middle Plate
%smiData.Solid(12).mass = 1.0;
%smiData.Solid(12).CoM = [0.0 0.0 0.0];
%smiData.Solid(12).MoI = [0.0 0.0 0.0];
%smiData.Solid(12).PoI = [0.0 0.0 0.0];
%smiData.Solid(12).color = [0.0 0.0 0.0];
%smiData.Solid(12).opacity = 0.0;
%smiData.Solid(12).ID = "";

%Chassis Plate
%Visual Properties - Simple Middle Plate
smiData.Solid(1).mass = 0.06;  % kg
smiData.Solid(1).CoM = [6.2019019999999996 -175.39327299999999 1.5];  % mm
smiData.Solid(1).MoI = [18.780000000000001 38.825000000000003 57.556000000000004];  % kg*mm^2
smiData.Solid(1).PoI = [0 0 -0.0050000000000000001];  % kg*mm^2
smiData.Solid(1).color = [0.647058824 0.647058824 0.647058824];
smiData.Solid(1).opacity = 0.349019608;
smiData.Solid(1).ID = "RDCD*:*797da2dcc90f5064dcd69baf";

%Engine
%Visual Properties - Simple Engine
smiData.Solid(2).mass = 0.09;  % kg
smiData.Solid(2).CoM = [0 0 0];  % mm
smiData.Solid(2).MoI = [0 0 0];  % kg*mm^2
smiData.Solid(2).PoI = [0 0 0];  % kg*mm^2
smiData.Solid(2).color = [0.647058824 0.647058824 0.647058824];
smiData.Solid(2).opacity = 1.000000000;
smiData.Solid(2).ID = "JOD*:*017ae92e7960008552915205";

%Copper Column
%Visual Properties - Simple copper Column
smiData.Solid(3).mass = 0.008;  % kg
smiData.Solid(3).CoM = [0 0 0];  % mm
smiData.Solid(3).MoI = [0 0 0];  % kg*mm^2
smiData.Solid(3).PoI = [0 0 0];  % kg*mm^2
smiData.Solid(3).color = [0.952941176 0.631372549 0.219607843];
smiData.Solid(3).opacity = 1.000000000;
smiData.Solid(3).ID = "JFD*:*8755a7546b559bc1b791d817";

%Expansion Board
%Visual Properties - Simple
smiData.Solid(4).mass = 0.250;  % kg
smiData.Solid(4).CoM = [0 0 0];  % mm
smiData.Solid(4).MoI = [0 0 0];  % kg*mm^2
smiData.Solid(4).PoI = [0 0 0];  % kg*mm^2
smiData.Solid(4).color = [0.231372549 0.380392157 0.705882353];
smiData.Solid(4).opacity = 1.000000000;
smiData.Solid(4).ID = "JHD*:*5c86d1c3299141bced3bf2c4";

%Copper Column
%Visual Properties - Simple
smiData.Solid(5).mass = 0.006;  % kg
smiData.Solid(5).CoM = [0 0 0];  % mm
smiData.Solid(5).MoI = [0 0 0];  % kg*mm^2
smiData.Solid(5).PoI = [0 0 0];  % kg*mm^2
smiData.Solid(5).color = [0.952941176 0.631372549 0.219607843];
smiData.Solid(5).opacity = 1.000000000;
smiData.Solid(5).ID = "JFD*:*659da5d64f4ec107c0ef89cd";

%Engine Mount
%Visual Properties - Simple
smiData.Solid(6).mass = 0.07;  % kg
smiData.Solid(6).CoM = [0 0 0];  % mm
smiData.Solid(6).MoI = [0 0 0];  % kg*mm^2
smiData.Solid(6).PoI = [0 0 0];  % kg*mm^2
smiData.Solid(6).color = [0.000000000 0.000000000 0.000000000];
smiData.Solid(6).opacity = 1.000000000;
smiData.Solid(6).ID = "JLD*:*017ae92e7960008552915205";

%Copper Column
%Visual Properties - Simple
smiData.Solid(7).mass = 0.006;  % kg
smiData.Solid(7).CoM = [0.39261999999999997 0.00041399999999999998 6.7000000000000002e-05];  % mm
smiData.Solid(7).MoI = [0.010999999999999999 0.16200000000000001 0.16200000000000001];  % kg*mm^2
smiData.Solid(7).PoI = [0 -0 0];  % kg*mm^2
smiData.Solid(7).color = [0.952941176 0.631372549 0.219607843];
smiData.Solid(7).opacity = 1.000000000;
smiData.Solid(7).ID = "JFD*:*204ce1af3c215416c7af658b";

%Battery
%Visual Properties - Simple
smiData.Solid(8).mass = 0.35;  % kg
smiData.Solid(8).CoM = [0 0 0];  % mm
smiData.Solid(8).MoI = [0 0 0];  % kg*mm^2
smiData.Solid(8).PoI = [0 0 0];  % kg*mm^2
smiData.Solid(8).color = [0.000000000 0.000000000 0.000000000];
smiData.Solid(8).opacity = 1.000000000;
smiData.Solid(8).ID = "JHD*:*a8348482368e5e07893e51e0";

%Foothold
%Visual Properties - Simple
smiData.Solid(9).mass = 0.035;  % kg
smiData.Solid(9).CoM = [6.196904 -2.6299549999999998 -246.89323499999998];  % mm
smiData.Solid(9).MoI = [7.1749999999999998 10.861000000000001 3.9790000000000005];  % kg*mm^2
smiData.Solid(9).PoI = [-0.47800000000000004 -0 -0];  % kg*mm^2
smiData.Solid(9).color = [0.647058824 0.647058824 0.647058824];
smiData.Solid(9).opacity = 0.349019608;
smiData.Solid(9).ID = "JUD*:*a2a519ca6b99d34d29d34999";

%Felge
%Visual Properties - Simple
smiData.Solid(10).mass = 0.03;  % kg
smiData.Solid(10).CoM = smiData.RigidTransform(53).translation / 25.4;  % in
smiData.Solid(10).MoI = [1e-3 1e-3 1e-3];  % lbm*in^2
smiData.Solid(10).PoI = [0 0 0];  % lbm*in^2
smiData.Solid(10).color = [0.792156863 0.819607843 0.933333333];
smiData.Solid(10).opacity = 1.000000000;
smiData.Solid(10).ID = "JFH*:*3326098a9e403299d41c1625";

%Reifen
%Visual Properties - Simple
smiData.Solid(11).mass = 0.09;  % lbm
smiData.Solid(11).CoM = smiData.RigidTransform(54).translation / 25.4;  % in
smiData.Solid(11).MoI = [0 0 0];  % lbm*in^2
smiData.Solid(11).PoI = [0 0 0];  % lbm*in^2
smiData.Solid(11).color = [0.792156863 0.819607843 0.933333333];
smiData.Solid(11).opacity = 1.000000000;
smiData.Solid(11).ID = "JFD*:*3326098a9e403299d41c1625";

%Middle Plate
%Visual Properties - Simple
smiData.Solid(12).mass = 0.04;  % kg
smiData.Solid(12).CoM = [6.2021860000000002 -177.45083600000001 1.5];  % mm
smiData.Solid(12).MoI = [16.317 38.470000000000006 54.741000000000007];  % kg*mm^2
smiData.Solid(12).PoI = [0 0 -0.0050000000000000001];  % kg*mm^2
smiData.Solid(12).color = [0.400000000 0.400000000 0.400000000];
smiData.Solid(12).opacity = 0.349019608;
smiData.Solid(12).ID = "RFCD*:*797da2dcc90f5064dcd69baf";


%============= Joint =============%
%X Revolute Primitive (Rx) %Y Revolute Primitive (Ry) %Z Revolute Primitive (Rz)
%X Prismatic Primitive (Px) %Y Prismatic Primitive (Py) %Z Prismatic Primitive (Pz) %Spherical Primitive (S)
%Constant Velocity Primitive (CV) %Lead Screw Primitive (LS)
%Position Target (Pos)

%Initialize the RevoluteJoint structure array by filling in null values.
smiData.RevoluteJoint(2).Rz.Pos = 0.0;
smiData.RevoluteJoint(2).ID = "";

smiData.RevoluteJoint(1).Rz.Pos = 0;  % deg
smiData.RevoluteJoint(1).ID = "[M/JqJcB5Lk7hNWPPR:-:Mi7OlT21hijGcVQe1:M/RGs/Ph4e0yE1FFn]";

smiData.RevoluteJoint(2).Rz.Pos = 0;  % deg
smiData.RevoluteJoint(2).ID = "[MI2dF8WfaHT4dwwx7:-:Mmd4uProBNDr4dXwC]";

